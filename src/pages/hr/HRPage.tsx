...
export default function HRPage({ branchFilter }: Props) {
  ...
  const canManageDepartments = profile?.role ? ['admin', 'manager', 'hr'].includes(profile.role) : false;
  ...
  const deleteDepartment = async (department: Department) => {
    if (!canManageDepartments) {
      setError('You are not allowed to manage departments');
      return;
    }

    if (!window.confirm(`Delete department "${department.name}" and remove all employee department assignments?`)) {
      return;
    }

    const { error: employeeClearError } = await supabase
      .from('employees')
      .update({ department_id: null })
      .eq('department_id', department.id);

    if (employeeClearError) {
      setError(employeeClearError.message);
      return;
    }
